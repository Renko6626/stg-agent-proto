...
static void test_player_and_scalars(void)
{
    ...
    reset_world();
    W.player.focus = 1;
    st = sa_model_fill(&W, -48.0f, 300.0f, 13, &IN);
    ...
    W.player.focus = 0;
    sa_model_fill(&W, 0.0f, 0.0f, 0, &IN);
...
{
    reset_world();
    sa_model_fill(&W, 0.0f, 0.0f, -5, &IN);
    assert(IN.prev_action[0] == 0);
    sa_model_fill(&W, 0.0f, 0.0f, 999, &IN);
    assert(IN.prev_action[0] == 0);
    sa_model_fill(&W, 0.0f, 0.0f, SA_MODEL_ACTIONS - 1, &IN);
...
static void test_bullet_columns_and_compaction(void)
{
    ...
    W.nbullets = 3;
    st = sa_model_fill(&W, 0.0f, 384.0f, 0, &IN);
...
static void test_enemy_columns(void)
{
    ...
    W.enemies[0].hit_w = 16.0f; W.enemies[0].hit_h = 24.0f;   /* hit_h 不进图（v2 只读 hit_w） */
